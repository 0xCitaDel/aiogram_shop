...
async def get_products_from_cart(db: Database, user_id: int):
    ...
    products_info = [(p.id, p.title, p.price, p.quantity, p.photo_id) for p in products]
    return products_info

async def check_and_add_to_cart(db: Database, user_id: int, product_id: int, quantity: int, price: int):
    await db.cart.add_product_in_cart(user_id, product_id, quantity, price)
    return product_id
